...
export default function UserProfileModal({ user, onClose, onLogout }) {
    const { accessToken, apiBase, updateUser } = useAuth();
    ...
    const renderHistoryItem = (item, idx, type) => {
        ...
        return (
            <div
                key={`${type}-${idx}`}
                className="border border-white/10 overflow-hidden transition-all"
            >
                <div
    ...
    return (
        ...
        >
            <div
                className="bg-black border border-white/10 w-full max-w-[700px] max-h-[85vh] overflow-y-auto"
                onClick={(e) => e.stopPropagation()}
            >
                {/* Header */}
                <div className="sticky top-0 bg-black border-b border-white/10 p-8 flex justify-between items-center">
                    <div>
